...
from bot_ui.formatting import (
    build_analyze_message,
    build_chain_choice_message,
    build_help_message,
    build_start_message,
)
from bot_ui.keyboards import chain_choice_keyboard, analyze_actions_keyboard
...
async def on_chain_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _render_analysis(query.message, evm, chain, address)
...
async def _render_analysis(message, evm, chain: str, address: str) -> None:
    try:
        snapshot = await evm.snapshot(chain, address)
    except Exception:
        log.exception("snapshot failed")
        try:
            await message.edit_text("⚠️ Provider temporarily unavailable\\. Please try again\\.")
        except Exception:
            pass
        return
